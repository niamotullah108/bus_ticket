...
char routes[TOTAL_ROUTES][100] = {
    "Chashara to Signboard", "Chashara to Gulisthan", "Earth to Mars        "};
...
int main() {
  ...
  while (1) {
    ...
    if (choice == 1) {
      ...
      printf("\t-> Press 'ctrl + c' anytime to terminate the program (lose "
             "runtime data).\n");
      printf("\t-> Press 'enter' to continue using the program (keep runtime "
             "data).\n");
      printf("\n\t");
      getchar();
      fflush(stdin);
...
int search_by_token(char *token) {
  ...
  {
    if (strcmp(token, c_data[i].token) == 0 &&
        c_existence[i] == 1) // check if match with any booked customer
    {
      printf("\tToken No.: %s\n", c_data[i].token);
      printf("\tName: \t%s\n", c_data[i].name);
  ...
  if (strcmp(pass, admin_pass) == 0) {
    ...
    while (1) {
      ...
      if (strlen(pass) < 4 || strlen(pass) > PASS_LEN - 1) {
        printf(
            "\tPassword must have to be between 4 and 12 characters long!\n\n");
        printf("\tPress 'enter' to continue!\n");
        printf("\t");
        getchar();
        fflush(stdin);
...
void delete_by_token(char *token) {
  ...
  if (c_existence[digit - 1] == 0) {
    ...
    for (int i = 0; i < c_data[digit - 1].n_seats; i++) {
      check_seat[c_data[digit - 1].booked_seats[i] - 1] =
          1; // set seat to available
    }
...
void view_booked_seats(int c_count) {
  if (c_count == 0) {
    ...
    printf("\n\t(Serial No. > Token No. > Booked by > Destination > No. of "
           "booked seats)\n\n");
    printf("\t-----------------------------------------------------------------"
           "-------\n\n");
    ...
    for (int i = 0, j = 0; i < TOTAL_SEATS && j < c_count; i++) {
      if (c_existence[i] == 1) {
        ...
        printf("\t\t%d. > %s > %s > %s > %d\n\n", (j + 1), c_data[i].token,
               c_data[i].name, c_data[i].destination, c_data[i].n_seats);
...
void book_seat() {
  ...
  printf("\n");
  /* idk what causing a bug here, which including a '>>' while entering multiple
  seats together to solve that i had to include this linebreak. which seems to
  work */
  ...
  while (1) {
    ...
    } else if (ch == 'n' || ch == 'N') // cancel booking
    {
      ...
      printf("\n\tInvalid input!\n\n");
      fflush(stdin);
      printf("\t(if it is Yes enter 'y' or 'Y' and if it is No enter 'n' or "
             "'N')\n\n");
